...
bool H264Encoder::initialize(const CaptureSettings& settings,
    ...
    av_opt_set_int(m_impl->swr_context, "in_channel_layout", av_get_default_channel_layout(channels), 0);
    av_opt_set_int(m_impl->swr_context, "out_channel_layout", m_impl->audio_codec_context->channel_layout, 0);
...
bool H265Encoder::initialize(const CaptureSettings& settings,
    ...
    m_impl->audio_codec_context->sample_rate = sample_rate;
    m_impl->audio_codec_context->channels = channels;
    m_impl->audio_codec_context->channel_layout = av_get_default_channel_layout(channels);
    m_impl->audio_codec_context->sample_fmt = AV_SAMPLE_FMT_FLTP;
    m_impl->audio_codec_context->time_base = {1, sample_rate};
    ...
    m_impl->audio_frame->format = m_impl->audio_codec_context->sample_fmt;
    m_impl->audio_frame->nb_samples = m_impl->audio_codec_context->frame_size;
    m_impl->audio_frame->channels = m_impl->audio_codec_context->channels;
    m_impl->audio_frame->channel_layout = m_impl->audio_codec_context->channel_layout;
